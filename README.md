...
Easy Connect pulls in all of the network drivers. As there is no global configuration to disable the compilation of unnecessary drivers, some of the code will get compiled and linked in, even though it is not used. So, if you are really tight on the binary size we recommend that you disable the configuration of unnecessary network drivers with a `.mbedignore` file located in the root of your application.
